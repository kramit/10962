Configuration A10962_VMs {
    Param(
        [Parameter(Mandatory=$True)]
        [PSCredential]$AdatumCredential,

        [Parameter(Mandatory=$True)]
        [PSCredential]$AdatumSafeModeCredential,
    ...
    # You should do LON-DC1 first, then the other two
    # This assumes that the computers already have the correct
    # computer names (LON-DC1, LON-SVR1, LON-CL1). The servers
    # can be Server Core or Server With a GUI.
    ...
    Node 'LON-DC1' {
        ...
        xADDomain Adatum {
            ...
            DomainAdministratorCredential = $adatumcredential
            SafemodeAdministratorPassword =  $AdatumSafeModeCredential
            DependsOn = '[WindowsFeature]ADDSInstall','[xIPAddress]IP','[xDNSServerAddress]DNS','[xComputer]Computer'
        }

        Archive HelpFiles {
            Path = "E:\AllFiles\PowerShellHelp\Help.zip"
            Destination = "C:\Help"
            Ensure = 'Present'
        }

        Script UpdateHelp {
            DependsOn = '[Archive]HelpFiles,[xADDomain]Adatum'
            GetScript = { @{} }
            TestScript = { Test-Path C:\HelpUpdated.txt }
            SetScript = { 
                Update-Help -SourcePath C:\Help\Help
                Get-Date | Out-File C:\HelpUpdated.txt 
            }
        }
    ...
    Node 'LON-CL1' {
        ...
        Environment Env {
            ...
            Ensure = 'Present'
            Path = $true
            Value = 'E:\AllFiles\WindowsPowerShell\Modules'
        }
		...
		File XMLInventory {
			SourcePath = "E:\AllFiles\Mod05\labfiles\inventory.xml"
			DestinationPath = "C:\inventory.xml"
			Ensure = "Present"
		}
...
A10962_VMs -OutputPath C:\A10962_VMs -ConfigurationData $ConfigurationData -EthernetInterfaceAlias 'Ethernet0'
Start-DscConfiguration -Path C:\A10962_VMs -ComputerName $env:COMPUTERNAME 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwIGUYDIJ/TfRcuB1wGrLeV51
# 962gghhlMIIEwzCCA6ugAwIBAgITMwAAAMlkTRbbGn2zFQAAAAAAyTANBgkqhkiG
...
# gjcCARUwIwYJKoZIhvcNAQkEMRYEFFIMZEuLA+UzRLLErbuHHhSUUrC+MF4GCisG
...
# SIb3DQEBAQUABIIBAFTWxOmXgJgqjA144qZuetedSQrsfWSyyPGJSoyUWMtXXD02
# IY267g+hGhBQeAeZ6T1Me0KM5y5jtfGPizESaJ138vT0M9kP13swA22B2jJQ/xbI
# lk0iOOpg/GAfwXvzfIgOLvJyIJeOcCm7BvhrgE7Jv7J9oObNCewwOsbhKz+eiUAW
# SL7orPVWOCH3kWyC5ixW9yCnhbPQbPaLPSAJKVdqfWWI3u+6kiGmefL9Jn5mP8Xt
# iMZC67EYLmsSKQ1vBK+WvEEbvHYlIXsn0H8rUaym2Ab33UJk40m9tIYI9JGsytyD
# lYmal+nPa9fduMPYM0nsBla87ElGaR0h/cdhVWWhggIoMIICJAYJKoZIhvcNAQkG
...
# BBTyC9lIV216NLp6UjRkilECzx57LjANBgkqhkiG9w0BAQUFAASCAQBVoz2KEIXk
# o735EwBDDcNXL5mTsr8P1kgmcSfNYUd96s7qK/feollKKpTw3PlzSjrcFq3anFiN
# qHkWQcucBIMaWxSV8Rt31/9+OOx3014a1/gkyeSuf21mRF29p7X5ZAw7gi2B6Yry
# Fy2EDwlt6R3Y7i+f5zjBjRlXsnko3LgDINohl19UcyVhF38ydcYERAAoJsYtfQwZ
# RMe2ROVl6SUUuHXt3QG69zk6smwxJRNfFostGGX0rH/nnAyyITAxkzJLBzWW0iEs
# COIGv8eA1E/k0Aj1WR0L6pwxkKUJqjsj8pG9DIv5EwP68bpYpRm9yS4Nj6eI8UNZ
# p+ygrobyVj18
