...
if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
	Write-Error "Not running PowerShell as Administrator. Right-click the PowerShell icon in the Taskbar, and select 'Run as Administrator.'"
	break
}

# check hotfix
if ((get-hotfix -id KB2883200 | measure).count -eq 0) {
	Write-Error "KB2883200 is missing and required."
	break
}

# check name
if ($env:computername -notin @('LON-CL1','LON-SVR1','LON-DC1')) {
	Write-Error "Local computer name not an expected value. LON-CL1, LON-SVR1, LON-DC1 expected."
	break
}

# get network interface alias
$ifAlias = Get-NetAdapter | Select -Expand ifAlias
if (($ifAlias | Measure).Count -gt 1) {
	Write-Host
	Write-Host "Multiple network interfaces were detected. Their names are:" -foreground green
	foreach ($if in $ifalias) {
		Write-Host " $if" -foreground green
	}
	$ifAlias = Read-Host "Enter the name of the actual Ethernet interface"
}

# get credentials
$DomainCredential = Get-Credential -UserName 'ADATUM\Administrator' -Message 'Enter Pa$$w0rd for password.'
$SafeModeCredential = Get-Credential -UserName 'Administrator' -Message 'Enter Pa$$w0rd for password.'

# enable remoting on Win8.1
if ((Get-WmiObject -Class Win32_OperatingSystem | Select -expand caption) -like '*Windows 8.1*') {
    Enable-PSRemoting -confirm:$false -SkipNetworkProfileCheck | Out-Null
}

# copy DSC resources
$DSCPath = Join-Path -Path 'E:\AllFiles' -ChildPath "DSCModules\*" 
Copy $DSCPath 'C:\Program Files\WindowsPowerShell\Modules' -Recurse | Out-Null

# configure LCM
C:\10962Config\LCM.ps1 | Out-Null

# start DSC configuration
Write-Warning "NOTE: 'Failed to Update Help' errors are normal."
C:\10962Config\10962A-Lab-Config.ps1 -Adatum $domaincredential -Safe $safemodecredential -Ethernet $ifAlias
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcxpPJ6SrXZCPbDTjfnwUdR/L
# GZegghhlMIIEwzCCA6ugAwIBAgITMwAAAMzLuBPrXXItRQAAAAAAzDANBgkqhkiG
...
# gjcCARUwIwYJKoZIhvcNAQkEMRYEFH4kvVao1De0YAx+p9ZBb+kHoNbPMF4GCisG
...
# SIb3DQEBAQUABIIBALPX7lMy2yMEy0wfWz6GiZ5pprUaNEIgll60sYnQ0MaI/ZIt
# bbvr2jCQ+3MdxQmHiJTqKToevLbui+TxunGlb3rW2bIFZ8eV9dEfeO6Oe7FJJMMo
# WMrIW4V8etsTZ8NcPPIdO/AW/OhrkcIb3bSBLUTVClxGLRnBS0PPP/USTmhBqnsY
# PjVzFoYd4IER//BoiJjfyRx8w3xmgzsKhrB19WWlD8t4nqy6FJ3Erna1iY2/8Drg
# rRDw6QYdWV4JZH6WR27uKFB06NhAJBX8QUMu3zRuGX3kcsY6V6PBSbFvZ4MhrH8E
# JNbKj/eI5bz/1/xajwcrEsLYUvIUawPFIUgagomhggIoMIICJAYJKoZIhvcNAQkG
...
# BBSrBH+7iR4+lPZRpa7dbPpXcWEOuTANBgkqhkiG9w0BAQUFAASCAQAMfxfdZV/l
# qIyvU4CNI6OIGIdDgp80ogixKaqGeQ81nB+8X+sjhvcjKXiZI+oXB5yAwQeaLlBh
# SMBXBRSE9ADwUTg/jO9Ihvc1teM7L8XX60Rp8Fl5akiP1jBZTxCtvc+DQLX/Z5ze
# yBEmW4gxSSkloxABUPfQDQouhn1cb/ag2aupS87xT4g6lMHK4UUGm9XKOwMa3ziO
# 0Pgo6EHfHU50NiyKu74dwS+iNEULSv7koyHaow8y0UlFlYkkj2+5g2gIZhuOAe+R
# 6ftdPnXMZB1x1hT+0Ux+23nl9vinT4zDssokBH9naHUdodCfIx0vMBli3mfa9QHe
# vUYwKVCBkvgE
