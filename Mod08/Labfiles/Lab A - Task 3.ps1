Workflow Restart-AdatumComputer {
    Param (
        [string[]]$ComputerName1,
        [string[]]$ComputerName2
    )
    Foreach -parallel ($computer1 in $ComputerName1) {
        Restart-Computer -PSComputerName $computer1 -Force -Wait
        $os1 = Get-CimInstance -ClassName Win32_OperatingSystem -PSComputername $computer1 -Property LastBootUptime,CSName
        Write-Output -InputObject "$($os1.CSName) last boot uptime is $($os1.LastBootUpTime)"
    }
    Foreach -parallel ($computer2 in $ComputerName2) {
        Restart-Computer -PSComputerName $computer2 -Force -Wait
        $os2 = Get-CimInstance -ClassName Win32_OperatingSystem -PSComputername $computer2 -Property LastBootUptime,CSName
...
# Restart-AdatumComputer -ComputerName1 $ComputerName1 -ComputerName2 $ComputerName2
# to test this workflow as a job, run:
# Restart-AdatumComputer -ComputerName1 $ComputerName1 -ComputerName2 $ComputerName2 -AsJob -JobName RestartAdatumComputer
# to retrieve results when running the workflow as a job, identify the job name and run:
# Receive-Job -Name RestartAdatumComputer
# SIG # Begin signature block
# MIIdpQYJKoZIhvcNAQcCoIIdljCCHZICAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUODs+H40QiZFDuECTJ+1AiMsr
# 26WgghhlMIIEwzCCA6ugAwIBAgITMwAAALWsfW2HayYRRwAAAAAAtTANBgkqhkiG
...
# gjcCARUwIwYJKoZIhvcNAQkEMRYEFDu1OrH+COw3SqCL8uU6VNczxPrJMF4GCisG
...
# SIb3DQEBAQUABIIBAMeI7g4pQB+cayE0OWqkedXwrmFOdgDnpaFo0pv1ULI2M18t
# tL9t61JDAMjN4EEt/ztMZb8HbpDZTnho+SEuYwuz4TuyfsQEY7QPKNPR6ivcDkqX
# lw9sTxzkrrFgsiztoBLatM5BT+8Nh4+aOWfFfJcXFX+rwehEhX07h5Y2qJUsa0MJ
# r79nK/Fv9UhvOmVH0C1e5JVF3UjlxsQr9/uStePh0cHHsbEMFqRhgtSQmx+YbhMl
# hcyxuXSG0FKvL/tYlQboKyI7bZvGzKtBz30iavAaJPyqjiBwb+J3mToffyxVmdsc
# 4Bw5pBK42fXyalPoVkF3rZza0Xff12EKjewmgxGhggIoMIICJAYJKoZIhvcNAQkG
...
# AQcBMBwGCSqGSIb3DQEJBTEPFw0xNzA3MTkwMjQ2MTlaMCMGCSqGSIb3DQEJBDEW
# BBQwhisIF93Aaqo4hBW1anGMUtKhTTANBgkqhkiG9w0BAQUFAASCAQAUJqwtOFqp
# f2HXgKWBO/tdxEHleE3YVVYExjPju8sqwCB8DGHL8Rh8qFboakWPZhHdERDieSZb
# j0GaLOMyJrkW4RhckdR4Cp4Eh0dFPBZIyrcRe7z3N05y59+xpwZhhuY6q63GzDMU
# YZRbdtemGD/uP8BqxkWUmYN7ntvWjV6Ydns5IbQVy9285+FAeWf33FFEcwAXdlYu
# 89Xsc4b5AYGzPQ+wdSD+kyW+zcZUWTgE8k+eT75CdjyweDkdcym+XJj1EDynkbWl
# Kfnjps+3Xa4cXGw22RAfZGCJlUd9ca4VHrA4Dtcd7MRWT56Sw1Dy0N74PC0MbhvO
# PFOMQw3auVyr
